...
func init() {
	RegisterComponent("confirm_close_", func(s *discordgo.Session, i *discordgo.InteractionCreate) {
...
func handleTicketClosure(s *discordgo.Session, channelID string, msg *discordgo.Message, saveLogs bool, ticket *db.Ticket) {
	...
	case <-time.After(20 * time.Second):
		...
		if saveLogs {
			...
			if err != nil {
				s.ChannelMessageSend(channelID, "Erro ao buscar mensagens do ticket. O canal será mantido ativo.")
				return
			}
		}
		...
		if err != nil {
			s.ChannelMessageSend(channelID, "Erro ao registrar o fechamento do ticket no banco de dados. O canal será mantido ativo.")
			return
		}
